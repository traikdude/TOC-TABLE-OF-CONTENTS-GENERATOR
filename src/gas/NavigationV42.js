...
function nav42ValidatePairs_(doc, pairs, cfg) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    pair.forwardPass =
      ...
      !nav42IsMalformedOpenUrl_(pair.forwardActualUrl) &&
    ...
    if (cfg.reciprocal) {
      pair.reversePass =
        ...
        !nav42IsMalformedOpenUrl_(pair.reverseActualUrl) &&
...
function nav42CanonicalDocUrl_(doc) {
  let docId = null;
  ...
  if (!docId && doc && typeof doc.getUrl === 'function') {
    try {
      docId = nav42ExtractDocId_(doc.getUrl());
    } catch (e) {}
  }
  if (!docId) {
    throw new Error('Unable to determine document ID for navigation URLs.');
  }
  return 'https://docs.google.com/document/d/' + docId + '/edit';
}

function nav42BookmarkUrl_(doc, tabId, bookmarkId) {
  const base = nav42CanonicalDocUrl_(doc);
  const tabPart = (tabId && String(tabId).trim()) ? '?tab=' + encodeURIComponent(tabId) : '';
  return base + tabPart + '#bookmark=' + encodeURIComponent(bookmarkId);
}
...
function nav42IsMalformedOpenUrl_(url) {
  if (!url) return false;
  const val = String(url);
  return val.indexOf('docs.google.com/open?tab=') !== -1 ||
         (val.indexOf('docs.google.com/open') !== -1 && val.indexOf('id=') === -1);
}

function nav42IsSameDocumentBookmarkUrl_(doc, url) {
  if (!url) return false;
  const value = String(url);
  if (value.indexOf('#bookmark=') === -1) return false;
  ...
  if (docId) {
    if (value.indexOf('/document/d/' + docId) !== -1) return true;
    if (value.indexOf('id=' + docId) !== -1) return true;
  }

  // Also recognize legacy / malformed same-document links generated by earlier versions
  // (e.g. docs.google.com/open?tab=...#bookmark=... or relative #bookmark=...)
  // so that rebuildNavigationForDocumentV42 can safely recognize and rewrite them
  if (value.indexOf('docs.google.com/open?tab=') !== -1) return true;
  if (value.indexOf('docs.google.com/open#bookmark=') !== -1) return true;
  if (value.indexOf('#bookmark=') === 0) return true;
...
function nav42ExtractDocId_(urlOrId) {
  ...
  const matchDoc = value.match(/\/document\/d\/([a-zA-Z0-9-_]+)/);
  if (matchDoc && matchDoc[1]) return matchDoc[1];
  const matchOpen = value.match(/[?&]id=([a-zA-Z0-9-_]+)/);
